...
const Worklist = ({ tasks, members, onAddTask, onUpdateTask }) => {
        ...
        <tbody>
          {tasks.map((task) => (
            <tr key={task._id}>
              ...
              <td>
                <select value={task.assignedTo._id} onChange={(e) => onUpdateTask(task._id, { assignedTo: e.target.value })}>
                  {members.length === 0 ? (
              ...
              <td>
                <button onClick={() => onUpdateTask(task._id, { status: 'Done' })} className="btn btn-success">Complete</button>
              </td>
            </tr>
          ))}
        </tbody>
