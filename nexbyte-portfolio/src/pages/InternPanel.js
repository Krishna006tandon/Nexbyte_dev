...
const InternPanel = () => {
  ...
  const fetchInternData = async () => {
    ...
    try {
      ...
      const [tasksData, diaryData, reportsData, notificationsData, resourcesData, presentationTopicsData, groupMeetingsData, teamData, internshipRes, internReportRes, internOfWeekRes, myWinsRes, microProjectsRes, newCertRes] = await Promise.all([
      ...
      } else if (internshipRes && internshipRes.certificateData) {
        ...
        <div className="content-body">
          ...
          {activeSection === 'certificate' && (
            <div className="certificate-section animate-fade-in">
              <div className="section-header">
                <h2><i className="fas fa-award"></i> My Certificate</h2>
                <p>View your official NexByte Core internship certificate</p>
              </div>
              <div className="certificate-content" style={{ display: 'flex', justifyContent: 'center', padding: '20px' }}>
                {certificateData ? (
                  <CertificateResultCard certificate={certificateData} />
                ) : (
                  <div className="empty-state" style={{ textAlign: 'center', padding: '40px', backgroundColor: '#F8FAFC', borderRadius: '12px', width: '100%' }}>
                    <i className="fas fa-file-contract" style={{ fontSize: '48px', color: '#cbd5e1', marginBottom: '16px' }}></i>
                    <h3>Certificate Not Available</h3>
                    <p style={{ color: '#64748b' }}>Your certificate has not been issued yet. It will appear here once it is generated by the administration.</p>
                  </div>
                )}
              </div>
            </div>
